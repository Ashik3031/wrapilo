...
export async function GET(
    ...
    try {
        ...
        console.log(`[API] Fetching assigned products for category: ${params.id}`);
...
export async function POST(
    ...
    try {
        ...
        console.log(`[API] Syncing products for category ID: ${params.id}. Expected count: ${productIds?.length}`);
        ...
        const targetCategory = await Category.findById(params.id);
        if (!targetCategory) {
            return NextResponse.json({
                success: false,
                error: `Category not found (ID: ${params.id})`
            }, { status: 404 });
        }

        // 1. Remove this category from products that ARE NOT in the list
        const removeResult = await Product.updateMany(
            { categories: params.id, _id: { $nin: productIds } },
